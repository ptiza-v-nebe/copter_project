...
int am_sw = 7; // Auto-Manual Switch
...
void initArduino(){
  ...
  pinMode(am_sw, INPUT_PULLUP); //Switch for switching from automatic and manual control

  //button debounce routine
  debouncer.attach(am_sw);
...
void loop(){
  ...
  //read from am_sw with debounce functionality
  debouncer.update();
  int command = debouncer.read();
  Serial.println(String("Command") + command);

  //set state
  if(!manual && command == HIGH){ 
    manual = true;
    Serial.println("joystick only mode");
  }
  if(manual && command == LOW){
    manual = false;
    Serial.println("joystick & imu mode");
  }
  ...
  Serial.println(String("State") + manual);
}
